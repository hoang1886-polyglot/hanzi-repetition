...
function hskRenderWordReader() {
      ...
      <div class="hsk-reader-card">
        ...
        <div class="hsk-card-header">
          ...
          <div class="hsk-card-actions">
            ...
            ${!isMastered ? `<button class="hsk-memorized-btn" id="hsk-memorized-btn" title="Đánh dấu là đã thuộc lòng, không cần ôn lại">🎓 Thuộc lòng rồi</button>` : ''}
            ${adminBtn}
          </div>
        </div>
  ...
  // Mark as memorized (no SRS needed)
...
function hskMarkMemorized(hskWord) {
  ...
  if (existing) {
    // Từ đã trong từ điển → nâng lên mastered luôn
    existing.status = 'mastered';
    existing.repetitions = 99;
    existing.ef = 2.5;
    existing.interval = 36500; // 100 năm — thực tế không cần ôn nữa
    existing.nextReview = Date.now() + 36500 * 86400000;
    existing.lastReview = Date.now();
  } else {
    // Chưa có trong từ điển → thêm thẳng với status mastered
    const book = hskGetBook(hskState.bookId);
    db.words.push({
      id: Date.now() + Math.random(),
      zh: hskWord.zh,
      ...
      source: `${book?.id || 'hsk'}-unit${hskState.unitIndex + 1}`,
      status: 'mastered',
      ef: 2.5, interval: 36500, repetitions: 99,
      nextReview: Date.now() + 36500 * 86400000,
      lastReview: Date.now(),
      added: Date.now(),
      memorizedDirectly: true,
    });
  }
  save();
