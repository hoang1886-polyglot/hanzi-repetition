...
function renderSvoResult(data) {
  ...
  segs.forEach((seg, i) => {
    ...
    const isMain = ['S','V','O'].includes(role);
    const block = document.createElement('div');
    block.style.cssText = [
      `flex:${isMain ? '1.2' : '0.7'}`,
      `background:${c.bg}`,
      `border-right:${i < segs.length - 1 ? '1px solid var(--border)' : 'none'}`,
      'padding:10px 8px',
      'display:flex',
    ...
    if (isMain) {
      ...
      block.innerHTML = `
        <span style="font-size:9px;color:var(--text3)">${seg.role}</span>
        <span style="font-size:15px;color:var(--text2);font-family:'Noto Sans SC',sans-serif">${seg.text}</span>
        <span style="font-size:9px;color:var(--text3)">${seg.pinyin}</span>`;
    }
  ...
  segs.filter(s => ['S','V','O'].includes(s.role)).forEach(seg => {
    ...
    card.style.cssText = `background:var(--surface);border:1px solid var(--border);border-left:3px solid ${c.border};border-radius:8px;padding:10px 12px;`;
    card.innerHTML = `
      <div style="font-size:10px;font-weight:700;color:${c.text};margin-bottom:3px">${c.label}</div>
      <div style="font-size:17px;font-weight:600;font-family:'Noto Sans SC',sans-serif">${seg.text}</div>
      <div style="font-size:11px;color:var(--text3)">${seg.pinyin}</div>
