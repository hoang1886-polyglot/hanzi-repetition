...
function renderArtReviewCard(){
  ...
  rc.innerHTML=`
    ...
    <div class="review-card">
      ...
      ${[...artReviewCard.zh].length>=2?`<div style="margin-top:10px"><button id="art-char-hint-btn" style="padding:6px 16px;background:linear-gradient(135deg,#3b9cf5,#1e6be5);border:none;border-radius:16px;font-size:12px;font-weight:700;color:#fff;cursor:pointer;font-family:'DM Sans',sans-serif;box-shadow:0 2px 8px rgba(30,107,229,0.25)">🔍 Gợi ý 1 chữ</button><div id="art-char-hint-display" style="display:none;margin-top:8px;padding:10px 16px;background:var(--surface2);border-radius:8px;border:1px solid var(--border);font-size:22px;font-family:'Noto Serif SC',serif;letter-spacing:0.15em;text-align:center"></div></div>`:''}
...
async function tbRenderArticle() {
  ...
  try {
    ...
    // Right panel tab switching — practice / write buttons toggle the panel
    let _artPanel = 'vocab';
    function _switchArtPanel(panel) {
      _artPanel = (_artPanel === panel) ? 'vocab' : panel;
      const practiceEl = $('tb-art-practice');
      const writeEl    = $('tb-art-write');
      const pbtn = $('tb-art-practice-btn');
      const wbtn = $('tb-art-write-btn');
      ...
      const RED = 'linear-gradient(135deg,var(--red),var(--red-dark))';
      if (pbtn) {
        const a = _artPanel === 'practice';
        pbtn.style.background = a ? RED : 'var(--surface)';
      ...
      if (wbtn) {
        ...
        wbtn.style.background = a ? RED : 'var(--surface)';
    ...
    $('tb-art-practice-btn').onclick = () => _switchArtPanel('practice');
    $('tb-art-write-btn').onclick    = () => _switchArtPanel('write');
...
function tbRenderArticleVocabPanel(words) {
  ...
  const notInDict = words.filter(w => !db.words.some(dw => dw.zh === w.zh));
  ...
  $('tb-add-all-vocab-btn')?.addEventListener('click', () => {
    let addedCount = 0;
    words.forEach((w, i) => {
      if (!db.words.some(dw => dw.zh === w.zh)) {
        const added = addWordFromTextbook(w.zh, w.vi||'', w.exZh||'', w.exVi||'', w.note||'');
        if (added) {
          addedCount++;
          const btn = vocabEl.querySelector(`.tb-vocab-add-btn[data-vi="${i}"]`);
          if (btn) {
            btn.textContent = '✓';
            btn.classList.add('added');
            btn.style.borderColor = 'var(--green)';
    ...
    if (addedCount > 0) {
      ...
      const addAllBtn = $('tb-add-all-vocab-btn');
      if (addAllBtn) {
