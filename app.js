...
const HSK_BOOK_INDEX = [
  ...
  { id: 'hsk2', title: 'HSK 2', level: 2, icon: '🌿', desc: '500 từ sơ cấp — phiên bản HSK mới 2021', file: './data/hsk2.json' },
...
async function hskLoadBook(id) {
  ...
  try {
    ...
    // Gắn thêm meta từ index (icon, desc, level nếu JSON không có)
...
async function hskNav(view, bookId = null, unitIndex = null, wordIndex = null) {
  ...
  // Nếu cần vào book cụ thể, load JSON trước rồi mới render
...
function hskRenderBooks() {
  ...
  if (!grid) return;
  // Dùng HSK_BOOK_INDEX để render danh sách sách (không cần load JSON trước)
