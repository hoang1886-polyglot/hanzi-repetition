...
// ── HSK Book Index (lazy-load từ JSON) ──────────────────────────────────────
const HSK_BOOK_INDEX = [
  { id: 'hsk1', title: 'HSK 1', level: 1, icon: '🌱', desc: '500 từ cơ bản — phiên bản HSK mới 2021',       file: './data/hsk1.json' },
  { id: 'hsk2', title: 'HSK 2', level: 2, icon: '🌿', desc: '772 từ sơ-trung cấp — phiên bản HSK 3.0',      file: './data/hsk2.json' },
  { id: 'hsk3', title: 'HSK 3', level: 3, icon: '🍀', desc: '1000 từ trung cấp — phiên bản HSK mới 2021',   file: './data/hsk3.json' },
];

// Cache: tránh fetch lại nhiều lần
const HSK_CACHE = {};

async function hskLoadBook(id) {
  if (HSK_CACHE[id]) return HSK_CACHE[id];
  const entry = HSK_BOOK_INDEX.find(b => b.id === id);
  if (!entry) return null;
  try {
    const res = await fetch(entry.file);
    if (!res.ok) throw new Error('HTTP ' + res.status);
    const data = await res.json();
    HSK_CACHE[id] = { ...entry, ...data };
    return HSK_CACHE[id];
  } catch(e) {
    console.error('Không load được sách HSK:', id, e);
    return null;
  }
}

// ── Helpers ───────────────────────────────────────────────────────────────────
function hskGetBook(id) { return HSK_CACHE[id] || null; }
...
async function hskNav(view, bookId = null, unitIndex = null, wordIndex = null) {
  ...
  // Load JSON nếu chưa có trong cache
  if (bookId && !HSK_CACHE[bookId]) {
    const loadingEl = $('hsk-units-grid') || $('hsk-books-grid');
    if (loadingEl) loadingEl.innerHTML = '<div style="padding:40px;text-align:center;color:var(--text3)">⏳ Đang tải dữ liệu...</div>';
    await hskLoadBook(bookId);
  }
...
function hskRenderBooks() {
  ...
  if (!grid) return;
  grid.innerHTML = HSK_BOOK_INDEX.map(entry => {
    const cached = HSK_CACHE[entry.id];
    const total  = cached ? cached.units.flatMap(u => u.words).length : '…';
    const units  = cached ? cached.units.length : '…';
    const added  = cached ? hskCountAdded(entry.id, null) : 0;
    const pct    = (cached && total > 0) ? Math.round(added / total * 100) : 0;
    return `
    <div class="hsk-book-card level-${entry.level}" data-book="${entry.id}" style="cursor:pointer">
      <span class="hsk-book-icon">${entry.icon}</span>
      <div class="hsk-book-name">${entry.title}</div>
      <div class="hsk-book-meta">${units} units · ${total} từ vựng</div>
      <div class="hsk-book-progress-bar">
        <div class="hsk-book-progress-fill" style="width:${pct}%"></div>
      </div>
      <div class="hsk-book-progress-text">${added > 0 ? added + '/' + total + ' từ đã thêm (' + pct + '%)' : 'Chưa học từ nào'}</div>
    </div>`;
  }).join('');
      ...
      <div class="hsk-reader-card">
        ...
        <div class="hsk-card-header">
          ...
          <div class="hsk-card-actions">
            ${isMastered
              ? `<button class="hsk-detail-add-btn added" disabled>🎓 Đã thuộc lòng</button>`
              : inDict
            ...
            ${!isMastered ? `<button class="hsk-memorized-btn" id="hsk-memorized-btn">🎓 Thuộc lòng rồi</button>` : ''}
            ${adminBtn}
          </div>
        </div>
...
// ── Mark word as memorized (no SRS needed) ───────────────────────────────────
function hskMarkMemorized(hskWord) {
  ...
  save();
  toast(`🎓 Đã đánh dấu "${hskWord.zh}" là thuộc lòng!`);
  hskRenderWordReader();
}
